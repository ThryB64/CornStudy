...
def _gdd_by_crop_year(temp_series: pd.Series, dates: pd.Series) -> pd.Series:
    """Cumulative GDD (base 10°C) reset per crop year (Apr 1 – Sep 30).

    GDD accumulates from April 1 to September 30 of each year, then resets.
    Outside the growing season the value is NaN.
    Never use cumsum() over the full series — it would grow indefinitely.
    """
    dt = pd.to_datetime(dates)
    result = pd.Series(np.nan, index=temp_series.index)
    for year in dt.dt.year.unique():
        mask = (dt.dt.year == year) & (dt.dt.month >= 4) & (dt.dt.month <= 9)
        idx = mask[mask].index
        if len(idx) == 0:
            continue
        daily_gdd = (temp_series.loc[idx].astype(float) - 10.0).clip(lower=0)
        result.loc[idx] = daily_gdd.cumsum().values
    return result
...
def _doy_zscore(s: pd.Series, dates: pd.Series, min_years: int = 5) -> pd.Series:
    ...
    for _doy, idx in grouped.groups.items():
...
def build_weather_belt_features(meteo: pd.DataFrame) -> pd.DataFrame:
    ...
    if "wx_belt_tmax_c" in out.columns:
        ...
        # Additional threshold: extreme heat >38C
        out["wx_belt_heat_days_38c_30"] = (out["wx_belt_tmax_c"] > 38).rolling(30, min_periods=20).sum()

    # Precipitation deficit: 14-day vs 60-day rolling mean (negative = drier than normal)
    if "wx_belt_prcp_mm" in out.columns:
        prcp = out["wx_belt_prcp_mm"].astype(float)
        out["wx_belt_rain_deficit_14d"] = prcp.rolling(14, min_periods=7).mean() - prcp.rolling(60, min_periods=30).mean()

    # Growing Degree Days accumulated within each crop year (Apr 1 – Sep 30, reset annually)
    # Uses belt avg temperature; GDD = (tavg - 10).clip(lower=0)
    if "wx_belt_tavg_c" in out.columns and "Date" in out.columns:
        out["wx_belt_gdd_accumulated"] = _gdd_by_crop_year(out["wx_belt_tavg_c"], out["Date"])
