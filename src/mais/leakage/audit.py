...
def _is_suspect_name(name: str) -> bool:
    ...
    if s.endswith(".1"):  # pandas merge collision artefact
        return True
    return s.lower() in {"unnamed: 0", "unnamed:_0", "index"}
...
def audit_features_targets(
    ...
    skip_future_dep_for: list[str] | None = None,
) -> LeakageAudit:
    ...
    skip_future_dep_for
        Feature names to exclude from check 4. Use for calendar/schedule features
        (``is_*``, ``days_to_*``) that are published in advance and are by design
        known before the event — their shifted correlation with short-horizon targets
        is economically valid, not a data leak.
    """
    audit = LeakageAudit()
    ...
    # Calendar/schedule features (is_*, days_to_*) are exempt: their dates are published
    ...
    for tcol in target_cols:
        ...
        y = merged[tcol]
        for fcol in feature_cols:
            if fcol not in merged or fcol in _skip_set:
                continue
            x = merged[fcol]
            base = _safe_corr(x, y)
