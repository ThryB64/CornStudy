"""Tests V70 — classification du canal de compression (offline)."""
from __future__ import annotations

import numpy as np
import pandas as pd

import mais.research.v70_path_classification as v70
...
def test_run_v70(tmp_path, monkeypatch):
    monkeypatch.setattr(v70, "V70_DIR", tmp_path)
    out = v70.run_v70_paths(_synthetic_master())
    assert out["version"] == "V70-PATH-CLASSIFICATION"
    if out["verdict"] != "TOO_FEW":
        assert "by_path" in out
        shares = [v["share"] for v in out["by_path"].values()]
        assert all(0.0 <= sh <= 1.0 for sh in shares)
        assert out["verdict"] in ("COMPRESSION_MOSTLY_CBOT_DRIVEN", "COMPRESSION_MIXED_OR_EMA_DRIVEN")
